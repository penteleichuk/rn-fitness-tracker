...
class ActivityHistory(private val reactContext: ReactApplicationContext) {
    ...
    fun writeWorkout(
        ...
        try {
            val activityKey = options.getString("key")
                ?: throw IllegalArgumentException("key expected")
            val sessionName = options.getString("name")
            ...
            Fitness.getSessionsClient(reactContext, account)
                ...
                .addOnSuccessListener { _: Void? -> promise.resolve(true) }
        ...
        try {
            ...
            if (!GoogleSignIn.hasPermissions(account, fitnessOptions)) {
                throw IllegalAccessException(
                    UNAUTHORIZED_GOOGLE_FIT + DataType.TYPE_ACTIVITY_SEGMENT.name
                )
            }

            Fitness.getHistoryClient(reactContext, account)
                .deleteData(request)
                .addOnSuccessListener { _: Void? -> promise.resolve(true) }
                .addOnFailureListener { e: Exception -> promise.reject(e) }
        } catch (e: Exception) {
    ...
    companion object {
        private const val UNAUTHORIZED_GOOGLE_FIT =
